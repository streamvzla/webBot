    ...
    public function query(Request $request)
    {
        ...
        try {
            ...
            if ($foundCode) {
                ...
                $subjectToTest = (string) $extractedCodeModel->subject;
                
                foreach ($publicSubjects as $pubSub) {
                    $pattern = str_replace('[email]', '*', $pubSub->subject);
                    if (\Illuminate\Support\Str::is($pattern, $subjectToTest) || \Illuminate\Support\Str::is('*' . $pattern . '*', $subjectToTest)) {
            ...
            Query::create([
                ...
                'result' => $result,
                'code_hash' => $foundCode ? Query::hashCode($extractedCodeModel->code ?? substr($extractedCodeModel->body ?? '', 0, 100)) : null,
