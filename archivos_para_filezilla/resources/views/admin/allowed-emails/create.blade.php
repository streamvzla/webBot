...
@section('title', 'Autorizar Correo - Panel de Administración')
@section('content')
<div class="max-w-4xl mx-auto space-y-8">
    ...
    <div class="ui-anim-in">
        ...
        <div class="ui-hero">
            <div>
                ...
                <h1 class="ui-hero-title">Autorizar Correo</h1>
                <p class="ui-hero-sub">Agrega uno o varios correos a la lista de permitidos.</p>
            </div>
        </div>
    </div>
    ...
    <form action="{{ route('admin.allowed-emails.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div class="ae-card ui-anim-in ui-delay-1">
            ...
            <div class="ae-card-body">
                <div style="display: grid; grid-template-columns: 1fr; gap: 1.5rem; margin-bottom: 1.5rem;">
                    <div>
                        <label for="emails" class="ui-label">Correos Electrónicos *</label>
                        <textarea id="emails" name="emails" required
                               class="ui-textarea"
                               rows="4"
                               placeholder="usuario1@dominio.com&#10;usuario2@dominio.com" autofocus>{{ old('emails') }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <select id="email_account_id" name="email_account_id" class="ui-select">
                            ...
                            @foreach($emailAccounts as $account)
                                <option value="{{ $account->id }}" {{ old('email_account_id') == $account->id ? 'selected' : '' }}>{{ $account->email }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
