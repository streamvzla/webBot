...
@section('title', 'Editar Plan de Franquicia - Panel de Administración')
...
<div class="max-w-4xl mx-auto space-y-8">
    ...
    <div class="ui-anim-in">
        ...
        <div class="ui-hero">
            <div>
                ...
                <h1 class="ui-hero-title">Editar Plan de Franquicia</h1>
                <p class="ui-hero-sub">Actualizando límites y características del plan: <strong style="color:white;">{{ $franchisePlan->name }}</strong></p>
            </div>
        </div>
    </div>
    ...
    <form action="{{ route('admin.franchise-plans.update', $franchisePlan) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="ae-card-body">
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1.5rem; margin-bottom: 1.5rem;">
                    <div>
                        ...
                        <input type="text" id="name" name="name" required value="{{ old('name', $franchisePlan->name) }}"
                               class="ui-input {{ $errors->has('name') ? 'ui-input-error' : '' }}"
                               placeholder="Ej: Plan Básico, Pro, Master...">
                    </div>
                    ...
                    <div>
                        ...
                        <div style="position:relative;">
                            ...
                            <input type="number" step="0.01" id="price" name="price" value="{{ old('price', $franchisePlan->price) }}"
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1.5rem;">
                    <div>
                        ...
                        <input type="number" id="max_clients" name="max_clients" min="1" value="{{ old('max_clients', $franchisePlan->max_clients) }}"
                    ...
                    <div>
                        ...
                        <input type="number" id="max_queries_per_day_per_client" name="max_queries_per_day_per_client" required min="1" value="{{ old('max_queries_per_day_per_client', $franchisePlan->max_queries_per_day_per_client) }}"
        ...
        <div class="ae-card ui-anim-in ui-delay-2">
            ...
            <div class="ae-card-body">
                ...
                <ul id="features_list" style="display:flex;flex-direction:column;gap:0.5rem;padding:0;margin:0;list-style:none;">
                    @if(is_array($franchisePlan->features))
                        @foreach($franchisePlan->features as $feature)
                            <li style="background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.08); border-radius:0.75rem; padding:0.75rem 1rem; display:flex; justify-content:space-between; align-items:center;">
                                <div style="display:flex;align-items:center;gap:0.75rem;">
                                    ...
                                    <span style="font-size:0.875rem;font-weight:600;color:white;">{{ $feature }}</span>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1.5rem;">
                    <div style="padding:1.5rem; border-radius:0.875rem; background:rgba(255,255,255,0.02); border:1px solid rgba(255,255,255,0.05); display:flex; align-items:center; gap:1.25rem;">
                        <label class="ui-toggle-wrap">
                            ...
                            <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $franchisePlan->is_active) ? 'checked' : '' }} class="ui-toggle-inp">
