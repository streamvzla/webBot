    ...
    <div class="ui-anim-in">
        ...
        <div class="ui-hero">
            <div>
                ...
                <h1 class="ui-hero-title">Editar Usuario (Staff)</h1>
                <p class="ui-hero-sub">Modificando el perfil y los accesos de <span style="color:#e9d5ff; font-weight:700;">{{ $user->username }}</span></p>
            </div>
        </div>
    </div>
            ...
            <div class="ae-card-body">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1.5rem;">
                    <div>
                        ...
                        <p class="ui-hint-msg">Solo llénalo si deseas cambiar la contraseña actual.</p>
                        @error('password')
                            <p class="ui-error-msg">
                                <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                {{ $message }}
            ...
            <div class="ae-card-body">
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1.5rem; margin-bottom: 1.5rem;">
                    <div>
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name', $user->name) }}"
                               class="ui-input" placeholder="Juan Pérez">
                    </div>

                    <div>
                        <label for="phone" class="ui-label">Teléfono</label>
                        <input type="text" id="phone" name="phone" value="{{ old('phone', $user->phone) }}"
                               class="ui-input" placeholder="+57 300 1234567">
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1.5rem;">
                    <div>
                        ...
                        <select id="role" name="role" required class="ui-input" onchange="togglePlan(this.value)">
                            <option value="admin" {{ old('role', $user->role) === 'admin' ? 'selected' : '' }}>Franquicia (Admin Dueño)</option>
                            <option value="user" {{ old('role', $user->role) === 'user' ? 'selected' : '' }}>Revendedor (Staff)</option>
                        </select>
                    </div>
                    
                    <div id="plan_container" class="{{ old('role', $user->role) === 'user' ? 'hidden' : 'block' }}">
                        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1.5rem;">
                            <div>
                                <label for="plan_id" class="ui-label">Plan de Franquicia</label>
                                <select id="plan_id" name="plan_id" class="ui-input">
                            ...
                            <div>
                                ...
                                <p class="ui-hint-msg">Al pasar esta fecha, la Franquicia y sus clientes quedarán en <strong>Modo Lectura</strong>.</p>
                            ...
                            <div>
                                ...
                                <input type="number" id="grace_days" name="grace_days" min="0" max="30"
                                       value="{{ old('grace_days', $user->grace_days ?? 0) }}"
            ...
            <div class="ae-card-body">
                <div style="padding:1.5rem; border-radius:0.875rem; background:rgba(255,255,255,0.02); border:1px solid rgba(255,255,255,0.05); display:flex; align-items:center; gap:1.25rem;">
                    <label class="ui-toggle-wrap">
                        ...
                        <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $user->is_active) ? 'checked' : '' }} class="ui-toggle-inp">
