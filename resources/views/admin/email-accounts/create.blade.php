...
@section('title', 'Nueva Cuenta de Correo - Panel de Administración')

@section('header', 'Nueva Cuenta de Correo')
@section('description', 'Configura una nueva cuenta IMAP')
...
<div class="max-w-2xl">
    <form action="{{ route('admin.email-accounts.store') }}" method="POST" class="form-card" style="display:flex;flex-direction:column;gap:1.25rem;">
        @csrf
        ...
        <div>
            ...
            <p class="form-hint">Normalmente es el mismo que el correo electrónico</p>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" id="imap_host" name="imap_host" required value="{{ old('imap_host', 'imap.hostinger.com') }}"
                    class="form-input"
                    placeholder="imap.servidor.com">
            </div>
            <div>
                <label for="imap_port" class="form-label">Puerto IMAP *</label>
                <input type="number" id="imap_port" name="imap_port" required value="{{ old('imap_port', '993') }}"
                    class="form-input">
            </div>
        </div>
        ...
        <div>
            ...
            <select id="imap_encryption" name="imap_encryption"
                class="form-select">
                <option value="ssl" {{ old('imap_encryption') === 'ssl' ? 'selected' : '' }}>SSL/TLS</option>
                <option value="tls" {{ old('imap_encryption') === 'tls' ? 'selected' : '' }}>TLS</option>
                <option value="" {{ old('imap_encryption') === '' ? 'selected' : '' }}>Ninguna</option>
            </select>
        </div>
            ...
            templateResult: function(user) {
                ...
                var $container = $(
                    '<div style="display: flex; justify-content: space-between; align-items: center;">' +
                        '<span style="color: white; font-weight: 500;">' + user.text + '</span>' +
                    '</div>'
                );
                return $container;
            }
