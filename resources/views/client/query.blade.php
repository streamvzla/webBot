...
<style>
    ...
    .query-hero-sub { color: rgba(148,163,184,0.65); font-size: 0.85rem; line-height: 1.4; }
    .status-dot {
    ...
    .glass-card:hover { border-color: rgba(168,85,247,0.3); transform: translateY(-2px); box-shadow: 0 12px 40px rgba(168,85,247,0.1); }
    .glass-card::before {
        content: '';
    ...
    .query-layout-grid { display: flex; flex-direction: column; gap: 1.25rem; }
    @media (min-width: 1024px) {
        .saas-container { max-width: 1000px; }
        .query-layout-grid { display: grid; grid-template-columns: 2fr 1fr; gap: 2rem; align-items: start; }
    }

    /* ===== STEP TIMELINE ===== */
    .step-group { position: relative; padding-left: 2.25rem; margin-bottom: 2rem; }
    .step-group:last-child { margin-bottom: 0; }
    .step-group:not(:last-child)::after {
        content: '';
        position: absolute; left: 0.65rem; top: 2.25rem; bottom: -2rem;
        width: 2px; background: rgba(168,85,247,0.1);
    }
    .step-header { margin-bottom: 1.1rem; }
    .step-dot {
        position: absolute; left: 0; top: 0;
        display: flex; align-items: center; justify-content: center;
        width: 1.4rem; height: 1.4rem;
        background: #111; border: 2px solid rgba(168,85,247,0.2); border-radius: 50%;
        color: #8b949e; font-size: 0.72rem; font-weight: 800;
        transition: all 0.4s cubic-bezier(0.4,0,0.2,1); z-index: 2;
    }
    .step-title { font-size: 1rem; font-weight: 700; color: #f8fafc; margin: 0; line-height: 1.4rem; }
    .step-group.active .step-dot {
        background: linear-gradient(135deg, #7c3aed, #ec4899);
        border-color: transparent; color: #fff;
        box-shadow: 0 0 20px rgba(168,85,247,0.6); transform: scale(1.2);
    }
    ...
    /* ===== PLATFORM GRID ===== */
    .platform-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(90px, 1fr)); gap: 0.75rem; }
    .platform-item {
        background: rgba(255,255,255,0.02);
    ...
    .platform-item:hover {
        ...
        transform: translateY(-4px); box-shadow: 0 8px 22px rgba(168,85,247,0.15);
    }
    .platform-item.selected {
        background: linear-gradient(145deg, rgba(168,85,247,0.15), rgba(236,72,153,0.05));
        border-color: rgba(168,85,247,0.6);
        box-shadow: 0 8px 25px rgba(168,85,247,0.25), inset 0 0 0 1px rgba(168,85,247,0.3);
        transform: translateY(-4px) scale(1.02);
    }
    .platform-img { width: 2.75rem; height: 2.75rem; border-radius: 0.75rem; object-fit: cover; box-shadow: 0 4px 15px rgba(0,0,0,0.5); transition: transform 0.3s cubic-bezier(0.16,1,0.3,1); }
    .platform-item.selected .platform-img { transform: scale(1.1); box-shadow: 0 8px 25px rgba(168,85,247,0.4); }
    .platform-name { font-size: 0.75rem; font-weight: 600; color: #94a3b8; transition: color 0.3s; line-height: 1.2; }
    .platform-item.selected .platform-name { color: #fff; font-weight: 800; }
    .platform-icon-fallback { width: 2.75rem; height: 2.75rem; background: rgba(168,85,247,0.1); border: 1px solid rgba(168,85,247,0.2); border-radius: 0.5rem; display: flex; align-items: center; justify-content: center; font-size: 1.25rem; }

    /* ===== EMAIL SELECT ===== */
    .saas-select-wrapper { position: relative; }
    .saas-select { width: 100%; background: rgba(255,255,255,0.02); border: 1px solid rgba(168,85,247,0.2); color: #f8fafc; padding: 0.9rem 1.25rem; border-radius: 1rem; font-size: 0.95rem; outline: none; transition: all 0.3s; cursor: text; }
    .saas-select:focus { border-color: #a855f7; background: rgba(168,85,247,0.05); box-shadow: 0 0 0 3px rgba(168,85,247,0.15); }
    .saas-select:disabled { opacity: 0.4; cursor: not-allowed; }
    .saas-dropdown-list { margin-top: 0.5rem; background: rgba(15,10,24,0.95); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); border: 1px solid rgba(168,85,247,0.3); border-radius: 1rem; max-height: 200px; overflow-y: auto; display: none; box-shadow: 0 10px 40px rgba(0,0,0,0.5); position: absolute; width: 100%; z-index: 100; }
    .saas-dropdown-list.show { display: block; animation: fade-in-up 0.2s ease; }
    .saas-dropdown-item { padding: 0.875rem 1.25rem; color: #e2e8f0; cursor: pointer; transition: background 0.2s; border-bottom: 1px solid rgba(255,255,255,0.05); font-size: 0.9rem; }
    .saas-dropdown-item:last-child { border-bottom: none; }
    .saas-dropdown-item:hover { background: linear-gradient(90deg, rgba(168,85,247,0.2), transparent); color: #fff; }
    .saas-select-icon { position: absolute; right: 1.25rem; top: 50%; transform: translateY(-50%); color: #a855f7; pointer-events: none; transition: transform 0.3s; }
    .saas-select-wrapper.open .saas-select-icon { transform: translateY(-50%) rotate(180deg); }
    ...
    .btn-submit-magic {
        ...
        color: white; font-weight: 800; font-size: 0.95rem; letter-spacing: 0.02em;
        padding: 1rem 1.75rem;
        box-shadow: 0 8px 25px rgba(168,85,247,0.4);
        transition: all 0.3s cubic-bezier(0.16,1,0.3,1);
        position: relative; overflow: hidden; width: 100%; cursor: pointer;
    }
    .btn-submit-magic::after { content: ''; position: absolute; inset: 0; background: linear-gradient(90deg, transparent, rgba(255,255,255,0.15), transparent); transform: translateX(-100%); transition: transform 0.5s; }
    ...
    .btn-submit-magic:disabled { opacity: 0.4; cursor: not-allowed; transform: none !important; }

    /* ===== LOADERS ===== */
    .loader-inline { display: flex; align-items: center; gap: 0.75rem; font-size: 0.85rem; color: #a855f7; font-weight: 600; margin-bottom: 0.75rem; }
    .spinner-small { width: 1.25rem; height: 1.25rem; border: 2px solid rgba(168,85,247,0.2); border-top-color: #a855f7; border-radius: 50%; animation: spin 0.8s linear infinite; }
    .fullscreen-loader { text-align: center; padding: 3.5rem 0; }
    .spinner-large { width: 3.5rem; height: 3.5rem; border: 3px solid rgba(168,85,247,0.1); border-top-color: #ec4899; border-radius: 50%; animation: spin 1s linear infinite; margin: 0 auto 1.25rem; }
    ...
    .result-card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 2px; background: linear-gradient(90deg, transparent, #34d399, transparent); border-radius: 1.75rem 1.75rem 0 0; }
    .result-icon { width: 4.5rem; height: 4.5rem; background: linear-gradient(135deg, rgba(16,185,129,0.2), rgba(16,185,129,0.05)); border: 1px solid rgba(16,185,129,0.3); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 1.25rem; color: #34d399; box-shadow: 0 0 30px rgba(16,185,129,0.2); }
    @keyframes counter-pop { 0% { transform: scale(0.9); opacity: 0; } 50% { transform: scale(1.05); } 100% { transform: scale(1); opacity: 1; } }
    .result-code-wrap { position: relative; display: inline-block; max-width: 100%; margin: 1.25rem 0; animation: counter-pop 0.5s cubic-bezier(0.16,1,0.3,1); }
    .result-code-glow { display: none; }
    .result-code-box {
        position: relative; z-index: 1;
        background: #000; border-radius: 1.25rem;
        padding: 1.25rem 2rem;
        font-family: 'Courier New', monospace;
        font-size: clamp(1.75rem, 8vw, 3rem);
        font-weight: 900; letter-spacing: 0.15em; color: #fff;
        cursor: pointer; transition: all 0.3s;
        box-shadow: inset 0 0 20px rgba(255,255,255,0.05), 0 0 30px rgba(168,85,247,0.15);
        word-break: break-all; max-width: 100%;
    }
    .result-code-box:hover { transform: scale(1.02); box-shadow: inset 0 0 20px rgba(255,255,255,0.05), 0 0 40px rgba(168,85,247,0.3); }

    /* ===== BOTÁ“N LINK VERIFICACIÁ“N â€” PREMIUM ===== */
    .btn-magic-link {
        display: inline-flex; align-items: center; justify-content: center; gap: 0.625rem;
    ...
    .btn-magic-link:hover { transform: translateY(-2px); box-shadow: 0 10px 32px rgba(16,185,129,0.55); }

    /* ===== BOTÁ“N NUEVA CONSULTA ===== */
    .btn-nueva-consulta {
    ...
    .btn-nueva-consulta:hover { background: rgba(168,85,247,0.1); border-color: rgba(168,85,247,0.4); color: white; transform: translateY(-1px); }

    /* ===== ERROR STATE ===== */
    .error-card { text-align: center; padding: 2.5rem 1rem; }
    .error-icon { width: 4.5rem; height: 4.5rem; border-radius: 50%; background: linear-gradient(135deg, rgba(239,68,68,0.2), rgba(239,68,68,0.05)); border: 1px solid rgba(239,68,68,0.3); display: flex; align-items: center; justify-content: center; margin: 0 auto 1.25rem; color: #ef4444; box-shadow: 0 0 30px rgba(239,68,68,0.2); }
    .btn-retry { background: rgba(255,255,255,0.05); border: 1.5px solid rgba(255,255,255,0.1); color: #e2e8f0; padding: 0.875rem 2rem; border-radius: 0.875rem; font-size: 0.9rem; font-weight: 700; cursor: pointer; transition: all 0.2s; }
    .btn-retry:hover { background: rgba(168,85,247,0.1); border-color: rgba(168,85,247,0.35); color: white; }

    /* ===== STATS WIDGET ===== */
    .premium-stats-widget { display: flex; flex-direction: column; gap: 1rem; }
    .premium-box { background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.07); border-radius: 1.5rem; width: 100%; backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); padding: 1.25rem; transition: border-color 0.3s, transform 0.3s, box-shadow 0.3s; display: flex; flex-direction: column; justify-content: center; }
    .premium-box:hover { border-color: rgba(168,85,247,0.3); box-shadow: 0 12px 40px rgba(168,85,247,0.1); transform: translateY(-2px); }
    .selected-platform-display { display: flex; flex-direction: column; align-items: center; gap: 0.75rem; }
    .selected-platform-icon { width: 4rem; height: 4rem; border-radius: 1.1rem; object-fit: cover; box-shadow: 0 8px 25px rgba(168,85,247,0.2); }
    .selected-platform-icon-fallback { width: 4rem; height: 4rem; border-radius: 1.1rem; background: linear-gradient(135deg, rgba(168,85,247,0.2), rgba(236,72,153,0.1)); display: flex; align-items: center; justify-content: center; font-size: 1.75rem; border: 1px solid rgba(168,85,247,0.2); }
    .selected-platform-name { color: #fff; font-weight: 800; font-size: 1.05rem; }
    .premium-stat-row { background: rgba(255,255,255,0.02); border: 1px solid rgba(168,85,247,0.1); border-radius: 0.875rem; padding: 1rem 1.1rem; display: flex; align-items: center; gap: 1rem; margin-bottom: 0.625rem; transition: background 0.3s, border-color 0.3s; }
    .premium-stat-row:hover { background: rgba(168,85,247,0.05); border-color: rgba(168,85,247,0.3); }
    .premium-stat-row:last-child { margin-bottom: 0; }
    .premium-stat-icon { width: 2.5rem; height: 2.5rem; background: rgba(168,85,247,0.1); border-radius: 0.65rem; color: #a855f7; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
    .premium-stat-icon svg { width: 1.35rem; height: 1.35rem; }
    .premium-stat-text { display: flex; flex-direction: column; }
    .premium-stat-label { color: rgba(148,163,184,0.8); font-size: 0.68rem; font-weight: 600; margin-bottom: 0.1rem; text-transform: uppercase; letter-spacing: 0.05em; }
    .premium-stat-value { color: #fff; font-size: 0.9rem; font-weight: 700; }
    ...
    @media (max-width: 640px) {
        .saas-container { padding: 1.25rem 0.875rem 4rem; }
        .query-hero { padding: 1rem; gap: 0.75rem; }
        .glass-card { padding: 1.25rem 1rem; border-radius: 1.25rem; }
        .step-group { padding-left: 2rem; margin-bottom: 1.5rem; }
        .step-group:not(:last-child)::after { left: 0.55rem; }
        .step-dot { width: 1.25rem; height: 1.25rem; font-size: 0.65rem; }
        .step-title { font-size: 0.95rem; }
        .platform-grid { grid-template-columns: repeat(3, 1fr); gap: 0.6rem; }
        .platform-item { padding: 0.875rem 0.5rem; border-radius: 0.875rem; gap: 0.5rem; }
        .platform-img { width: 2.25rem; height: 2.25rem; }
        .platform-name { font-size: 0.7rem; }
        .result-card { padding: 2rem 1rem; border-radius: 1.25rem; }
        .result-code-box { padding: 1rem 1.25rem; letter-spacing: 0.08em; }
        .result-icon { width: 3.5rem; height: 3.5rem; }
        .btn-magic-link { max-width: 100%; padding: 0.875rem 1.25rem; font-size: 0.9rem; }
        .btn-nueva-consulta { width: 100%; }
        .error-card { padding: 2rem 0.5rem; }
        .error-icon { width: 3.5rem; height: 3.5rem; }
        .premium-stats-widget { display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem; }
        .premium-box { padding: 1rem; border-radius: 1.1rem; }
    }

    /* ===== MOBILE MUY PEQUEÁ‘O â‰¤380px ===== */
    @media (max-width: 380px) {
        .platform-grid { grid-template-columns: repeat(3, 1fr); gap: 0.5rem; }
        .platform-img { width: 2rem; height: 2rem; }
        .platform-name { font-size: 0.65rem; }
        .glass-card { padding: 1rem 0.875rem; }
        .premium-stats-widget { grid-template-columns: 1fr; }
    }
</style>
@endsection
...
<div class="saas-container">
    ...
    {{-- ===== RESULT: EMAIL FOUND EN SESIÁ“N ===== --}}
    @if(session('email_body') && session('temp_code_expiry') && now()->timestamp < session('temp_code_expiry'))
        ...
        <div class="result-card">
            ...
            @php
                $extracted = session('extracted_code');
                $codeType = $extracted['type'] ?? 'html';
                $codeValue = $extracted['value'] ?? session('email_body');
            @endphp

            @if($codeType === 'code')
                <p id="codeTitleLabel" style="font-size:0.75rem;color:#94a3b8;text-transform:uppercase;letter-spacing:0.1em;font-weight:600;">Tu Código Mágico (Clic para copiar)</p>
            ...
            @elseif($codeType === 'link')
                <p style="font-size:0.75rem;color:#94a3b8;margin-bottom:1rem;text-transform:uppercase;letter-spacing:0.1em;font-weight:600;">Haz clic para autorizar o continuar</p>
                <a href="{{ $codeValue }}" target="_blank" class="btn-magic-link">
                    <svg style="width:1.2rem;height:1.2rem;" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg>
                    Abrir Enlace de Verificación
                </a>
            @else
                <div style="text-align:center;padding:1.5rem 0;">
                    <div class="btn-nueva-consulta" style="cursor:default; border-color:rgba(168,85,247,0.3); color:#a855f7;">
                        ...
                        Correo recibido correctamente. Revisa las instrucciones de tu plataforma.
                    </div>
                </div>
            @endif

            <div style="text-align:center; margin-top:2rem;">
                <a href="{{ route('client.query.clear') }}" class="btn-nueva-consulta" style="margin-top:1.5rem;">
                    <svg style="width:1.1rem;height:1.1rem;" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg>
                    Nueva Consulta
                </a>
                <div style="font-size:0.85rem;color:#64748b;margin-top:1rem;">
                    Expira en <span id="countdown" data-remaining="{{ session('temp_code_expiry') - now()->timestamp }}" style="color:#f8fafc;font-weight:700;"></span>
                </div>
            </div>
        </div>
    ...
    @else
        ...
        <div class="query-layout-grid">
            ...
            <div id="errorState" class="error-card" style="display:none;position:relative;z-index:2;">
                ...
                <button type="button" onclick="resetForm()" class="btn-retry">
                    Intentar de Nuevo
                </button>
            </div>
            </div>
            ...
            <div class="premium-box">
                <div class="premium-stat-row">
                    ...
                    <div class="premium-stat-text">
                        ...
                        <div class="premium-stat-value" id="cooldownText">Listo para consultar</div>
                    </div>
                </div>
                ...
                <div class="premium-stat-row">
                    ...
                    <div class="premium-stat-text">
                        ...
                        <div class="premium-stat-value" id="queryCountText">0 de 100</div>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    window.selectPlatform = function(id, name, logoUrl = '') {
        ...
        fetch(`{{ route('client.emails.by-platform') }}?platform_id=${id}`)
            .then(res => res.json())
            .then(data => {
                ...
                if (data.success && data.emails && data.emails.length > 0) {
                    ...
                    _availableEmailsData = data.emails;
                    _availableEmails = data.emails.map(e => e.email);
                    renderDropdownList(_availableEmails);

                    // Auto-skip
                    if (data.emails.length === 1) {
                        select.value = data.emails[0].email;
                        checkFormReady();
                    }
                } else {
                    select.placeholder = 'Sin correos asignados';
                    select.disabled = true;
                }
            })
            .catch(() => {
                document.getElementById('email-loader').style.display = 'none';
                select.placeholder = 'Error al cargar';
            });
    }
    ...
    function updateLimits() {
        fetch(`{{ route('client.query.limit') }}`)
            .then(res => res.json())
            .then(data => {
                if (data.limit_status) {
                    const status = data.limit_status;
                    
                    // Actualizar Cooldown
                    _cooldownSeconds = status.seconds_until_next;
                    const cooldownText = document.getElementById('cooldownText');
                    ...
                    const queryText = document.getElementById('queryCountText');
                    queryText.textContent = `${status.used_today} de ${status.max_daily}`;
                    if (status.used_today >= status.max_daily) {
                        queryText.style.color = '#f87171';
                    }
                }
            })
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // checkSystemStatus eliminado
        updateLimits(); // Cargar límites reales al iniciar
    });
